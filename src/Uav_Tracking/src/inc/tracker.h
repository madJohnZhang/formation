...
public:
	...
	bool init(Mat frame, vector<int> bounding_box)
	{
		...
		if (para_.bg_size.x > para_.img_size.x)
			para_.bg_size.x = para_.img_size.x - 1;
		if (para_.bg_size.y > para_.img_size.y)
			para_.bg_size.y = para_.img_size.y - 1;
	...
	bool track(Mat const frame)
	{
		...
		if (para_.scale_adaption)
		{
			...
			if (para_.is_first_frame)
				para_.init_score = max_val[i];
			else if (max_val[i] < para_.init_score * para_.lost_threshold)
				lost_count++;
			else
				lost_count = 0;
	...
	//judge if the target is in the center of the image
	bool isCenter()
	{
		}
};
