...
fstream obs_distance("obs_distance.csv", ios::trunc | ios::out);
...
std::vector<float> obs_dis = vector<float>(5);
float safe_dis = 5;
float k_obs = 0.01;
...
int my_callback(int data_type, int data_len, char *content)
{
    ...
    if (e_obstacle_distance == data_type && NULL != content)
    {
        ...
        for (int i = 0; i < CAMERA_PAIR_NUM; ++i)
        {
            ...
            obs_distance << 0.01f * oa->distance[i] << ",";
            obs_dis[i] = oa->distance[i] * 0.01f;
        }
        obs_distance << endl;
        printf("\n");
...
void initObs(float safe = 10, float k = 1)
{
    safe_dis = safe;
    k_obs = k;
...
vector<float> getObstacleDis()
{
    //g_event.wait_event();
    for (auto i : obs_dis)
    {
        std::cout << " qqqq" << i << std::endl;
    }
    return obs_dis;
}

//2-D Obstacle Avoidance
vector<float> getAvoidanceInput()
{
    //down front right rear left
    //horizontal
    vector<float> obs = getObstacleDis();
    float dis_right = obs[2];
    float dis_left = obs[4];
    vector<float> D2result(2, 0);
    if (dis_right < safe_dis)
    {
        D2result[0] = pow((safe_dis - dis_right), 2);
    }
    if (dis_left < safe_dis)
    {
        D2result[0] = D2result[0] - pow((safe_dis - dis_left), 2);
    }
    D2result[0] *= k_obs;
    D2result[0] *= -1;
    return D2result;
